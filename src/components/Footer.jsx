...
const FinCoachLogo = () => (
  <svg width="160" height="44" viewBox="0 0 160 44" xmlns="http://www.w3.org/2000/svg">
    ...
    <polyline points="11,26 20,20 29,13" stroke="#5dcaa5" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="29" cy="13" r="2.5" fill="#5dcaa5"/>
    <text x="48" y="30" fontFamily="system-ui, sans-serif" fontSize="22" fontWeight="500" fill="#ffffff">Fin</text>
    <text x="76" y="30" fontFamily="system-ui, sans-serif" fontSize="22" fontWeight="400" fill="#5ba8e8">Coach</text>
    <rect x="48" y="34" width="18" height="10" rx="3" fill="#185FA5" fillOpacity="0.2" stroke="#185FA5" strokeWidth="0.8"/>
    <text x="57" y="42" fontFamily="system-ui, sans-serif" fontSize="8" fontWeight="500" fill="#5ba8e8" textAnchor="middle">AI</text>
  </svg>
);

const links = {
  Product: [{ label: "Dashboard", href: "/dashboard" }],
  Company: [
...
const Footer = () => {
  return (
    <footer className="relative bg-[#06090f] overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 sm:px-8 pt-16 pb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 mb-12">
          ...
          <div className="col-span-2 sm:col-span-3 lg:col-span-2">
            ...
            <p className="text-sm text-slate-500 leading-relaxed max-w-[260px] mb-6">
              AI-powered finance coaching that helps you track, plan, and grow your wealth — personalised to you.
            </p>
            <div className="flex items-center gap-3">
              {[
                { label: "Twitter", href: "#", icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M4 4l16 16M4 20L20 4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg> },
                { label: "GitHub", href: "#", icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 00-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0020 4.77 5.07 5.07 0 0019.91 1S18.73.65 16 2.48a13.38 13.38 0 00-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 005 4.77a5.44 5.44 0 00-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 009 18.13V22" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
                { label: "LinkedIn", href: "#", icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><circle cx="4" cy="4" r="2" stroke="currentColor" strokeWidth="1.8"/></svg> },
              ].map((s) => (
                <Link key={s.label} href={s.href} className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all">
                  {s.icon}
                </Link>
              ))}
            </div>
          ...
          {Object.entries(links).map(([group, items]) => (
            <div key={group}>
              <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-slate-600 mb-4">{group}</div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-white/[0.05] gap-3">
          <p className="text-xs text-slate-600">© {new Date().getFullYear()} FinCoach AI. All rights reserved.</p>
