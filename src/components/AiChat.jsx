  ...
  return (
    <div className={`flex gap-2.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
        isUser
          ? "bg-blue-700"
          : "bg-gradient-to-br from-[#185FA5] to-[#5dcaa5]"
      }`}>
        {isUser
...
const AiChat = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Hey! I'm your FinCoach AI 👋 I have access to your full financial data. Ask me anything — like whether you can afford something, how your spending looks, or where to save more.",
    },
  ]);
  const bottomRef = useRef(null);
  const inputRef = useRef(null);

  const { fn: sendMessage, loading } = useFetch(chatWithAI);
  ...
  const handleSend = async (text) => {
    ...
    // Get chat history (exclude the initial greeting)
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed bottom-20 right-6 z-50 w-[380px] flex flex-col rounded-2xl overflow-hidden border border-white/[0.1]"
          style={{
            background: "#0a1017",
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg bg-white/[0.05] border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-white transition-colors"
            >
              <X size={13} />
            </button>
          </div>
              ...
              <div className="space-y-2 pt-1">
                ...
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    className="block w-full text-left text-xs text-slate-400 bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2 hover:bg-white/[0.06] hover:text-slate-200 hover:border-white/[0.1] transition-all"
                  >
                    {s}
                  </button>
                ))}
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading
                    ? "linear-gradient(135deg,#185FA5,#1a7cc7)"
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-2xl flex items-center justify-center transition-all hover:scale-105 active:scale-95"
        style={{
          background: open
          ...
          boxShadow: open
            ? "none"
            : "0 8px 32px rgba(24,95,165,0.5), 0 0 0 1px rgba(91,168,232,0.3)",
        }}
      >
        {open
